...
const AnimatedBackground = () => {
  ...
  const updateKeyboardTransform = async () => {
    ...
    keycaps.forEach(async (keycap, idx) => {
      ...
      // Attempt to dynamically update the text and hide the old SVG logo on each keycap
      const skill = Object.values(SKILLS).find(s => s.name === keycap.name);
      if (skill && (keycap as any).children) {
        (keycap as any).children.forEach((child: any) => {
          const childName = (child.name || "").toLowerCase();
          // Hide old baked-in SVG logos
          if (childName.includes("logo") || childName.includes("icon") || child.type === "Vector" || child.type === "Image") {
            child.visible = false;
          }
          // Update the physical 3D text to the new skill name
          if (child.type === "Text" || childName.includes("text")) {
            // Keep text short so it fits on the key
            const shortName = skill.label.length <= 6 ? skill.label : skill.label.substring(0, 5) + ".";
            child.text = shortName;
            child.content = shortName; // some versions of spline use content
          }
        });
      }
